#!/usr/bin/env python
"""P2 观测期汇总：聚合窗口日报并输出 GO/NO_GO。窗口天数从 release_policy.yaml 读取，默认 3。"""
...
SUMMARY_PATH = os.path.join(REPORT_DIR, "p2_observation_summary.json")
...
def _load_daily_reports():
    ...
    files = sorted(
        ...
        if f.startswith("p2_day_") and f.endswith(".json")
    )
    reports = []
    for name in files:
...
def main():
    ...
    p0_count = sum(1 for r in reports if r.get("p0_detected", False))
    p1_count = sum(1 for r in reports if r.get("p1_detected", False))
    ...
    summary = {
        "stage": "p2_observation_finalize",
        "window_days": WINDOW_DAYS,
        "days_collected": days_collected,
        "p0_count": p0_count,
        "p1_count": p1_count,
        "all_days_passed": all_days_passed,
    ...
    print()
    print("=" * 50)
    print(f"  P2 观测汇总  ({WINDOW_DAYS}-day window)")
    ...
    if final_decision == "GO":
        print(f"  最终判定: GO — B 版准入条件满足")
    elif days_collected < WINDOW_DAYS:
        print(f"  最终判定: NO_GO — 仅 {days_collected}/{WINDOW_DAYS} 天，不足窗口")
    else:
        reasons = []
        if p0_count > 0:
        ...
        print(f"  最终判定: NO_GO — {', '.join(reasons)}")
    print("=" * 50)
    print()
