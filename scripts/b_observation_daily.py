#!/usr/bin/env python
"""B 轨每日观测：执行 release_gate / runtime_healthcheck / rollback_verify 并产生 B 轨日报。"""
...
def _next_day_index(date_utc):
    if not os.path.exists(DAILY_DIR):
        return 1
    today_file = f"b_day_{date_utc}.json"
    files = [f for f in os.listdir(DAILY_DIR) if f.startswith("b_day_") and f.endswith(".json")]
...
def main():
    ...
    rg_path = os.path.join(DAILY_DIR, f"b_release_gate_{date_utc}.json")
    hc_path = os.path.join(DAILY_DIR, f"b_healthcheck_{date_utc}.json")
    rb_path = os.path.join(DAILY_DIR, f"b_rollback_{date_utc}.json")
    ...
    daily_report = {
        "b_stage": "b_observation_daily",
        "b_day_index": day_index,
        "b_date_utc": date_utc,
        "b_release_gate_passed": release_gate_passed,
        "b_healthcheck_passed": healthcheck_passed,
        "b_rollback_verify_passed": rollback_verify_passed,
        "b_p0_detected": p0_detected,
        "b_p1_detected": p1_detected,
        "b_final_decision": final_decision,
        "b_timestamp_utc": timestamp,
    }

    if error_message:
        daily_report["b_error_message"] = error_message

    daily_path = os.path.join(DAILY_DIR, f"b_day_{date_utc}.json")
...
def _run_json_script(label, cmd, out_path):
    ...
    with open(out_path, "r", encoding="utf-8") as f:
        return result.returncode == 0, json.load(f)
...
def _print_visual_summary(day_index, window_days, gate_ok, health_ok, rollback_ok,
    ...
    if p0:
        status_line = "B当前判定: NO_GO (P0 detected)"
    elif p1:
        status_line = "B当前判定: NO_GO (P1 detected)"
    elif pending > 0:
        status_line = f"B当前判定: GO (pending {pending} more day(s))"
    else:
        status_line = "B当前判定: GO (observation complete)"
    ...
    print()
    print("=" * 50)
    print(f"  B P2 Day {day_index}/{window_days}  {bar}  {pct}%")
    ...
    print(f"  B目标结束: {target_date.strftime('%Y-%m-%d')}")
    print("=" * 50)
    print()
