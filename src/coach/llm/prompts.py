...
格式模板:
  方向 | A[标签] -->|边标签| B{标签} | B-->|边标签| C[标签]
  用 | 分隔方向/节点/边。方向: LR(横向,2-4节点) 或 TD(纵向,层级)。节点标签格式"概念名: 解释≤15字"。边标签≤5字动词。

示例（按格式模板填写，不写 Mermaid 代码）:

  例1 矩阵概念:
  "flowchart LR | A[矩阵: m×n矩形阵列] -->|按形状分类| B{n=m?} | B-->|是| C[方阵: 行列式/逆矩阵] | B-->|否| D[非方阵: 转置/秩]"

  例2 函数调用:
  "flowchart TD | A[调用函数] --> B[传参: 实参复制给形参] | B --> C[执行函数体] | C --> D[返回结果: return值]"

  例3 循环结构:
  "flowchart TD | A[初始化循环变量] --> B{条件判断} | B-->|满足| C[执行循环体] | C --> B | B-->|不满足| D[退出循环]"

  例4 对比分类:
  "flowchart LR | A[数据结构] -->|按可变性分| B{可变?} | B-->|是| C[列表: append/remove] | B-->|否| D[元组: 不可修改]"

触发条件:
- 引入新概念/教操作步骤/对比分类 → 必须输出 _diagram_plan
- 不确定也输出最简版: "flowchart LR | A[概念名] --> B[要素1] | A --> C[要素2]"

豁免（不输出 _diagram_plan）:
- 探测轮/情绪支持/简短确认/纯追问细节
- 节点超15字如"矩阵是一个按行列排列的数字矩形阵列非常有用" → 精简
- 12 个节点每个子概念独立 → 精简到 3-6 个
- content 为空 → 不确定时画最简单的 A[概念]-->B[要点]
- 探测轮画图 → 豁免
...
- "topics": 涉及的知识点列表
- "_diagram_plan": 引入新概念/教操作/对比分类时必须输出。格式: "方向 | A[标签] -->|边标签| B{标签} | ..."。方向LR或TD, 节点≤15字, 边标签≤5字, 3-6节点
- "diagram": 画图模型从 _diagram_plan 自动生成此字段，不要手动填写。格式: {"type":"mermaid","content":"..."}""",
    "scaffold": """
【脚手架引导模式 - 输出要求】
...
- 【教学自查】生成前逐条核对: ①是否先用 KaTeX 展示了具体数字实例？只给抽象定义 → 必须补实例。新手不能从抽象学起 ③提问是否开放式、有区分度（不能人人都答对/答错）？ ⑤教操作/运算 → 是否展示了具体数字的变换前后对比？ ⑥涉及易混淆概念 → 是否并排对比了差异？
- 本轮是否应该先问学生的已有理解？如果这是本话题的第一轮（上一轮没有问过探测题），且引入的是新概念 → 必须先探测。如果上一轮已经探测过了（不管学生如何回答），本轮禁止再问"你接触过X吗""你了解X吗""你用过X吗"等探测句式——直接进入教学
- 【教学用图】本轮是否引入新概念/教操作/对比分类？是 → 输出 _diagram_plan（不要填 diagram 字段——画图模型会自动生成）。探测轮/情绪支持/简短确认 → 豁免。不确定也输出最简版
