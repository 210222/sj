...
- **全量回归**：每个阶段合并前必须 `python -m pytest tests/ -q` 且 1363+ tests 全部 pass
- **源码边界**：`src/inner/**`、`src/middle/**`、`src/outer/**` 禁止修改；新代码只允许在 `src/coach/`、`src/mapek/`、`src/cohort/`
- **合约冻结规则**：新合约一旦标记为 frozen，后续只能新增字段不可修改已有字段

### 当前阶段

- **活动阶段**: Phase 31 — 全线收尾（上下文引擎重做 + 接线闭环 + 体验修复）
- **已完成阶段**: Phase 0-31 全部 GO ✅
- **全量测试**: 1363 passed / 7 failed / 5 skipped（7 个失败为 Phase 27 上下文重构已知回归，非 P0）
