...
### 方案产出逐项比对（每次 Phase 完毕后强制执行）

每次完成代码修改后，必须回到原始方案/要求，逐项比对：
1. 列出方案中的每一项要求
2. 标注每项状态：✅ 已完成 / ❌ 未完成 / ⚠️ 部分完成
3. 未完成项立即修复，不得遗留到下一轮对话
4. 全量回归通过 + 比对表全部 ✅ 后，才能宣称"完成"
...
├── meta_prompts/coach/          # XML 元提示词 (Phase 0-62)
├── data/                        # SQLite 运行时数据
...
- **活动阶段**: Phase 73 — 全栈缺陷修复 + 双模型画图协同 + Flow 回路闭合
- **已完成阶段**: Phase 0-73 全部 GO ✅
- **全量测试**: 1466 passed / 0 failed / 5 skipped
- **教学评分基线**: fake_understanding 23.0, misconception 20.0, anxious 14.0（LLM-as-judge 30 分制）
- **缓存资格**: 100% eligible，stable_prefix_share 31.08%
- **白金审计**: 健康指数 53.4，WARN（预存 S30/S50/S65/S80 需 API key）
- **已启用能力**: TTM, SDT, Flow, LLM, MAPE-K, MRT, diagnostic_engine, diagnostics, excursion

### Phase 72-73 核心交付

| Phase | 名称 | 核心交付 |
|-------|------|----------|
| **72** | 双模型画图协同 | DeepSeek _diagram_plan 描述 + Gemini 生成 Mermaid。双路兼容（diagram 直出+Gemini 优化）。终端清单注意力重排（10 项去重，按触发频率×质量影响排序） |
| **73** | 全栈缺陷修复 | 5 bugs（探测轮重复/内容追问重复/KaTeX JSON 破损/WebSocket 断连/Flow 回路闭合）。4 架构优化（Admin 真实数据/CoachBridge 缓存/跨进程文件锁/MRT 配置读取）。6 prompt 工程（辅导协议跨轮边界/探测闭环三分支/学生反馈处理三分类/终端清单重构）。白金审计 F003 SQL 注入修复。Excursion 启用。__pycache__ 清理。 |
...
| **42** | 前端/APK 对齐 | API 类型同步、ChatBubble LLM 标签（延迟/缓存/成本）、Admin 客户端 |
| **43** | APK + Admin 收尾 | build_apk.bat 就绪、config 恢复、Admin 数据管道、全量 0 failed |
| **44** | 交互式教学审计 | LLM 学生代理（3 画像）、interactive 模式、mastery_delta 量化、4 维效果评分 |
| **45** | 治理层缺口封版 | P0 关闭（31/37/39 XML+completion）、P1 关闭（38/43）、P2 债务登记册 |
| **46** | 全量缺口最终封版 | S45 未完成项封版、债务文档批量、S44.3 效果评分实现、全量 0 failed |
| **47** | 代码级债务封版 | 13 P0/P1 修复（persistence 列索引/Admin 断链/yaml 防护/WAL/recal/前端类型） |
| **48** | 教学能力评测 | 7 维评测框架 + 学生代理 v2（3 挑战模式）+ --teaching-audit |
| **49** | 辅导能力升级 | _STABLE_SYSTEM_PREFIX 注入 6 条辅导协议（Graesser/Chi/Lepper/Dweck） |
| **50** | LLM 学生代理 | _generate_student_response() API key + 纯文本调用 + _build_student_prompt() |
| **51** | 辅导终端注入 | _render_terminal_tutoring_checklist() — 辅导自检位于 decoding frontier |
| **52** | LLM 评委 | evaluate_with_llm() — 同行评议式评估 (temperature=0.1)，揭示关键词假阴性 |
| **53** | 稳定性与精细调优 | 多跑取均值 (--runs N)、验证优先+对比示例、turns 6→10、教练 28.0/30 (4.7/5.0) |
| **54** | 治理层封版 | Phase 47-53 completion + XML 补齐、CLAUDE.md 同步 |
| **55** | LLM Observability 可视化 | LLMRuntimeCard、Admin 面板(缓存/延迟/教学审计柱状图)、reports 静态挂载 |
| **56** | 文档缺口封版 | Phase 50-53 XML orchestrator + Phase 55 completion。全 55 Phase 闭环 |
| **57** | 治理收口 | Phase 55-56 XML + completion + CLAUDE.md + 交接记忆更新 |
| **58** | 学习路径规划 | SkillGraph 8→18 + _check_learning_path() + 中文映射 + context 注入 |
| **59** | 学习地图可视化 | /config 挂载 + SkillTreeCard (绿/黄/灰) + App.tsx 集成 |
| **60** | Vite 代理修复 | /config + /reports 路径代理到 8001 |
| **61** | 新用户体验修复 | 无 mastery 数据时只显示提示不显示 19 节点 + 审计报告 7 天过期 |
| **62** | 图文教学能力 | Mermaid + KaTeX + Desmos + Prism。教练可选输出 diagram 字段。触发策略三模式 |
